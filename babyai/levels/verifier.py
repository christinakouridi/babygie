...
Obj -> "obj"
PP -> P NP
VP -> TV NP IV | TV NP PT
P -> "by" | "next_to" | "alongside" | "adjacent_to" | "to_the_side_of"
IV -> "ends_up" | "moves" | "is_placed" | "is_positioned" | "shifts"
TV -> "ensure" | "confirm" | "make_sure" | "make_certain"
PT -> "is_placed" | "is_moved" | "is_shifted" | "is_transferred"
V -> "put" | "place" | "move" | "shift"  
Det -> "the" | "my"
N -> "ball" 
Adj -> "blue"
"""
...
class Paraphraser:
    def __init__(self):
        ...
        self.token = self.cfg.start()

    def generate_sample(self, grammar, token, sample):

        if token in grammar._lhs_index:  # Derivation
            derivations = grammar._lhs_index[token]

            derivation = random.choice(derivations)
            for d in derivation._rhs:
                self.generate_sample(grammar, d, sample)
        elif token in grammar._rhs_index:
            # terminal
            token_str = str(token).replace('_', ' ')
            sample.append(token_str)
        return sample
    ...
    def find_matching_objs(self, env, use_location=True):
        ...
        for i in range(env.grid.width):
            for j in range(env.grid.height):
                ...
                if self.type != None and cell.type != self.type:
                    continue
...
class PutNextInstr_n(ActionInstr):
    """
    Put an object next to another object
    eg: put the red ball next to the blue key
    """

    def __init__(self, obj_move, obj_fixed, strict=False, test_instr_mode='original', test_grammatical_vocab=None, test_random_vocab=None):
        super().__init__()
        assert obj_move.type != 'door'
        self.desc_move = obj_move
        self.desc_fixed = obj_fixed
        ...
        self.test_grammatical_vocab = test_grammatical_vocab
        self.test_random_vocab = test_random_vocab

    def surface(self, env):
        if self.test_instr_mode == 'rand_vocab':
            vocab = ['key', 'ball', 'box', 'blue', 'green', 'yellow', 'grey', 'purple', 'red', 'to', 'the']

            obj1 = self.desc_move.surface(env).split(' ')
            obj2 = self.desc_fixed.surface(env).split(' ')

            vocab.remove(obj1[1])
            vocab.remove(obj2[2])

            return ' '.join(random.sample(vocab, 9))

        elif self.test_instr_mode == 'original':
            return 'put ' + self.desc_move.surface(env) + ' next to ' + self.desc_fixed.surface(env)
        elif self.test_instr_mode == 'false_permuted':
            return 'put ' + self.desc_fixed.surface(env) + ' next to ' + self.desc_move.surface(env)
        elif self.test_instr_mode == 'false':
            colours = ['blue', 'green', 'yellow', 'grey', 'purple', 'red']
            objects = ['key', 'ball', 'box']

            ent1 = self.desc_move.surface(env).split(' ')
            ent2 = self.desc_fixed.surface(env).split(' ')

            col1, obj1 = ent1[1], ent1[2]
            col2, obj2 = ent2[1], ent2[2]
            ...
            cols = random.sample(colours, 2)
            col1, col2 = cols[0], cols[1]

            return 'put the ' + col1 + ' ' + obj1 + ' next to the ' + col2 + ' ' + obj2
        elif self.test_instr_mode == 'text_inverted':
            return 'next to ' + self.desc_fixed.surface(env) + ' put ' + self.desc_move.surface(env)
        elif 'paraphrased' in self.test_instr_mode:
            tokens = []
            tokens = PARAPHRASER.generate_sample(PARAPHRASER.cfg, PARAPHRASER.cfg.start(), tokens)
            parts = ' '.join(tokens).split('obj')

            if self.test_instr_mode == 'paraphrased':
                return parts[0] + self.desc_move.surface(env) + parts[1] + self.desc_fixed.surface(env)
            elif self.test_instr_mode == 'paraphrased_permuted':
                return parts[0] + self.desc_fixed.surface(env) + parts[1] + self.desc_move.surface(env)
            else:
                raise ValueError(f"Incorrect test instruction mode name: {self.test_instr_mode}")
        elif 'random' in self.test_instr_mode:

            randn_word = lambda n: ' '.join(random.sample(self.test_random_vocab, n)).lower()
            randn_word_grammatical = lambda n: ' '.join(random.sample(self.test_grammatical_vocab, n)).lower()

            if self.test_instr_mode == 'semi_random_grammatical':
                part_0 = ' ' + randn_word_grammatical(1) + ' '
                part_1 = ' ' + randn_word_grammatical(2) + ' '
                return part_0 + self.desc_move.surface(env) + part_1 + self.desc_fixed.surface(env)
            elif self.test_instr_mode == 'random_grammatical':
                return ' ' + randn_word_grammatical(9) + ' '

            elif self.test_instr_mode == 'semi_random_agrammatical':
                parts_0 = ' ' + randn_word(1) + ' '
                parts_1 = ' ' + randn_word(2) + ' '
                return parts_0 + self.desc_move.surface(env) + parts_1 + self.desc_fixed.surface(env)

            elif self.test_instr_mode == 'random_agrammatical':
                return ' ' + ' '.join(randn_word(1) for i in range(9)) + ' '
