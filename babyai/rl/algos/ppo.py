...
class PPOAlgo(BaseAlgo):
    ...
    def update_parameters(self):
        ...
        missions_raw = exps.get('obs').get('instr')[3].tolist()

        if self.savelog_missions:
            seen_missions = [' '.join(mission) if isinstance(mission, Iterable) else mission for mission in missions_raw]
        ...
        for _ in range(self.epochs):
            ...
            for inds in self._get_batches_starting_indexes():
                ...
                for i in range(self.recurrence):
                    ...
                    if 'gie' in self.acmodel.lang_model or 'bert' in self.acmodel.lang_model:
                        sb = DictList()

                        obs_dict = DictList()
                        obs_dict.image = exps.get('obs').image[inds + i]
                        obs_dict.instr = (exps.get('obs').instr[0][inds + i],
                                          exps.get('obs').instr[1][inds + i],
                                          exps.get('obs').instr[2][inds + i],
                                          exps.get('obs').instr[3][inds + i])
                        sb.obs = obs_dict

                        sb.memory = exps.get('memory')[inds + i]
                        sb.mask = exps.get('mask')[inds + i]
                        sb.action = exps.get('action')[inds + i]
                        sb.value = exps.get('value')[inds + i]
                        sb.reward = exps.get('reward')[inds + i]
                        sb.advantage = exps.get('advantage')[inds + i]
                        sb.returnn = exps.get('returnn')[inds + i]
                        sb.log_prob = exps.get('log_prob')[inds + i]
                    else:
                        sb = exps[inds + i]
        ...
        if self.savelog_missions:
            logs["seen_missions"] = seen_missions
