...
class RoomGrid(MiniGridEnv):
    ...
    def add_distractors_train_test(self, i=None, j=None, num_distractors=2, all_unique=True,
                                   num_instr_objs=1, pairs_dict=None, test_mode=False):

        assert pairs_dict
        # pairs_dict = utils.train_test_splits(seed=1)

        if test_mode == True:
            instr_pairs = pairs_dict['test_pairs']
            other_pairs = pairs_dict['train_pairs']
        else:
            instr_pairs = pairs_dict['train_pairs']
        ...
        # Select objects to be part of the instruction and add to the environment
        # Collect the gym representation of objects
        objs_gym = []
        while len(objs_gym) < num_instr_objs:

            obj = self._rand_elem(instr_pairs)
            ...
            obj_gym, pos = self.add_object(room_i, room_j, *obj)

            objs.append(obj)
            objs_gym.append(obj_gym)

        # Collect a list of existing instruction and distractor objects
        d_objs = objs.copy()

        while len(d_objs) < num_distractors + num_instr_objs:

            if test_mode == True:
                d_obj = self._rand_elem(instr_pairs + other_pairs)
            else:
                d_obj = self._rand_elem(instr_pairs)

            if all_unique and d_obj in d_objs:
                continue
            ...
            _, _ = self.add_object(room_i, room_j, *d_obj)
            d_objs.append(d_obj)
